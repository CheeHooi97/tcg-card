...
func (r *cardRepository) GetByCardNameAndSet(cardName, set string) bool {
	var auth model.Card
	result := r.db.
		Where("name = ? AND setName = ?", cardName, set).
		First(&auth)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return false
...
func (r *cardRepository) SearchCardBySort(sortType, userId string) ([]*model.Card, error) {
	...
	case "momentum_up", "momentum_down":
		...
		err := r.db.
			Table("cardprice").
			Select("cardId, price").
			Where("createdDateTime <= DATE_SUB(NOW(), INTERVAL 7 DAY)").
		...
		for _, r := range rows {
			if _, exists := weekAgoPrice[r.CardID]; !exists {
				weekAgoPrice[r.CardID] = r.Price
			}
		}
	...
	case "relevance":
		...
		err := r.db.
			Table("usercardsearchlog").
			Select("cardId").
			Where("userId = ?", userId).
			Order("createdDateTime DESC").
			Limit(100).
			Pluck("cardId", &ids).Error
...
// 				Joins(`
// 			JOIN (
// 				SELECT cardId, COUNT(*) AS search_count
// 				FROM usercardsearchlog
// 				WHERE createdDateTime >= DATE_SUB(NOW(), INTERVAL 7 DAY)
// 				GROUP BY cardId
// 			) ucs ON ucs.cardId = cards.id
// 		`).
...
// 				SELECT
// 					cardId,
// 					MAX(CASE WHEN createdDateTime >= DATE_SUB(NOW(), INTERVAL 7 DAY) THEN price END) -
// 					MIN(CASE WHEN createdDateTime < DATE_SUB(NOW(), INTERVAL 7 DAY) THEN price END) AS price_change
// 				FROM ungrade
// 				GROUP BY cardId
// 			) cp ON cp.cardId = cards.id
// 		`).
